...
$resultFile = $submission->files->firstWhere('file_type', 'result');
@endphp
<x-layouts.app title="Detail Permohonan">
    ...
    <x-card class="p-6 sm:p-8 mb-6">
        ...
        @if($isRejectedOrExpired)
            <x-alert :type="$submission->status === 'REJECTED' ? 'danger' : 'warning'">
                ...
                @if($submission->note)
                    <p class="mt-1">{{ $submission->note }}</p>
                @endif
            </x-alert>
        @else
            <ol class="space-y-0">
    ...
    @if($submission->status === 'APPROVED' && $resultFile)
        <x-card class="p-6 sm:p-8 bg-emerald-50 border-emerald-200">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <x-link-button :href="URL::temporarySignedRoute('files.show', now()->addMinutes(15), ['file' => $resultFile->id])" variant="success" size="lg">
